...
class DatabaseManager:
    ...
    def select_ip_port_from_proxypool(self):
        try:
            ...
            print(e)
            return []
    def select_ip_port_type_from_proxypool(self):
        try:
            # 连接到proxypool_db数据库
            conn = sqlite3.connect(self.proxypool_db)
            ...
            # 查询所有记录的 IP 和 Port
            cursor.execute("SELECT ip, port, type FROM proxypool")
            ...
            print(e)
            return []
    def select_ip_lasttime_from_proxypool(self):
        ...
        try:
            ...
            # 查询所有记录的 IP 和 Port
            cursor.execute("SELECT ip, lasttime FROM proxypool")
    ...
    def delete_all_from_proxypool(self):
        try:
            makesure = input("Are you sure to delete all proxies?(y/n)")
            if makesure == "y" or makesure == "":
                ...
                # 删除符合条件的记录
                cursor.execute("DELETE FROM proxypool")
                ...
                print("Please make sure next time!")
        except Error as e:
            print(e)
    ...
    def select_proxypool_num(self):
        ...
        try:
            ...
            # 查询所有符合条件的记录的 URL
            cursor.execute("SELECT count(*) FROM proxypool where is_useful = 1")

            result = cursor.fetchone()[0]

            print("proxypool共有{}条有效记录".format(result))
